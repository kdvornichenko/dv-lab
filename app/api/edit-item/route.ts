...
export async function POST(request: Request) {
    try {
        ...
        const updatedItems = currentItems.map(item =>
            item.id === itemId
                ? { ...item, price, description, href }
                : item
        )

        // Если item не найден, добавляем новый
        if (!currentItems.find(item => item.id === itemId)) {
            updatedItems.push({ id: itemId, price, description, href })
        }
        ...
        if (image) {
            ...
            const blob = await put(`items/${itemId}.${image.name.split('.').pop()}`, image, {
                access: 'public',
            })
            console.log('Изображение загружено:', blob.url)
        }
