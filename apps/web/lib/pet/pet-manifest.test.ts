...
for (const pose of PET_POSES) {
	...
	assert.equal(typeof animation.loop, 'boolean', `Animation ${pose} must define loop behavior`)
}

assert.equal(catPetManifest.id, 'cat')
assert.equal(catPetManifest.animations.sleep.src, '/assets/cat-widget/cat-sleep.webp')
assert.equal(catPetManifest.animations.sleep.width, 370)
assert.equal(catPetManifest.animations.sleep.height, 200)
assert.equal(catPetManifest.animations.sleep.naturalFrameCount, 5)
...
assert.equal(catPetManifest.animations.lie.src, '/assets/cat-widget/cat-walk-to-lay.webp')
assert.equal(catPetManifest.animations.lie.width, 370)
assert.equal(catPetManifest.animations.lie.height, 200)
assert.equal(catPetManifest.animations.lie.naturalFrameCount, 6)
assert.equal(catPetManifest.animations.lie.loop, false)
assert.equal(catPetManifest.animations.stretch.src, '/assets/cat-widget/cat-lay-to-walk.webp')
assert.equal(catPetManifest.animations.stretch.width, 370)
assert.equal(catPetManifest.animations.stretch.height, 200)
assert.equal(catPetManifest.animations.stretch.naturalFrameCount, 6)
assert.equal(catPetManifest.animations.stretch.loop, false)
